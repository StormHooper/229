...
def affine_encrypt(text, a, b):
  ...
  for letter in text:
    if letter.isalpha():
      # Used util.py to initialize 'num' to be
      ...
      # Used util.py to append to the cipher the ENCRYPTED letter
...
def affine_decrypt(ciphertext, a, b):
  ...
  a_inv = pa2.mod_inv(a, 26)  # a_inv holds the inverse of a under modulo 26
  ...
  for letter in ciphertext:
    if letter.isalpha():
      ...
      # Used util.py to find the integer `num` that corresponds
      # to the given letter
      ...
      # Used util.py to append to the text the decrypted
...
def encryptRSA(plaintext, n, e):
  ...
  # Used util.py to initialize 'digits' as a string of
  # the two-digit integers that correspond to the letters of 'text'
  digits = util.letters2digits(text)

  # Used util.py to initialize 'l' with the length of each RSA block
  l = util.blocksize(n)

  # Used a loop to pad 'digits' with enough 23's (i.e. X's)
  ...
  for b in blocks:
    ...
    if len(encrypted_block) < l:
      ...
      encrypted_block = '0' + encrypted_block[0:]
